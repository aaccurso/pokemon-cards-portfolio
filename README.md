...
- **Shipment import** — paste a Cardmarket order email and the app parses items, fuzzy-matches to cards on your wishlist, flags duplicates, and saves the log.
...
- **Shipment import only works locally.** The server action writes to `data/purchases.json`, and Vercel's production runtime has a read-only filesystem. On the deployed site the "Import Shipment" button will throw. See [`CLAUDE.md`](CLAUDE.md) for planned fixes.
